...
from main import NeuralNetwork, ImageDataLoader
...
def analyze_learning_rate_dependency(
    ...
    for lr in learning_rates:
        ...
        auto_encoder = NeuralNetwork(constant_lr_func)
        auto_encoder.init_weights(3 * r * m, compression_coef)

        loader = ImageDataLoader(image_path, r, m)

        iterations = 0
        for epoch, epoch_error in auto_encoder.train(loader, max_iterations, target_error):
            iterations = epoch
...
if __name__ == '__main__':
    analyze_learning_rate_dependency('pic/1024.bmp', r=10, m=10, target_error=500, max_iterations=200, learning_rates=[0.0095, 0.007, 0.0035, 0.001, 0.00075, 0.0005, 0.00025])
    analyze_compression_coef_dependency('pic/chessboard.bmp', r=10, m=10, target_error=750, compression_coefs=[4, 7, 10, 13, 16, 19])
    analyze_error_threshold_dependency('pic/image.bmp', r=10, m=10, max_iterations=200, compression_coef=10)
    ...
    analyze_training_progress('pic/1024.bmp')
    analyze_training_progress('pic/chessboard.bmp')
    analyze_training_progress('pic/image.bmp', r=10, m=10)
